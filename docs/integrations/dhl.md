...
All transitions require optimistic concurrency, evidence, and an idempotency key. External events are unique by `(source, event_id)` as well as idempotency key. A duplicate is resolved before validating the aggregate's already-advanced or terminal state and must match the durably persisted original machine, prior state, action, external identity, idempotency key, and destination, then return the matching durably persisted transition result with recursively immutable replay metadata and no repeated side effect. A duplicate without that durable result, or any illegal, stale, unsupported, mismatched, or under-evidenced new transition, fails closed.
