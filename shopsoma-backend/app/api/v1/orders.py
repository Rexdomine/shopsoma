...
async def review_order(
    ...
    shipping_query = select(ShippingRate).where(
        and_(
            ...
            or_(
                ...
                ShippingRate.min_order_value <= subtotal
            ),
            or_(
                ShippingRate.max_order_value == None,
                ShippingRate.max_order_value >= subtotal
            )
        )
    ...
    recommended_rate = next((r for r in shipping_rates if r.is_default), shipping_rates[0])
    shipping_cost = recommended_rate.base_rate

    # Calculate discount (if promo code provided)
    discount_amount = Decimal("0.00")
    applied_promo = None
    if review_data.promo_code:
        # TODO: Implement promo code validation and discount calculation
        # For now, hardcode a simple example
        if review_data.promo_code.upper() == "SAVE10":
    ...
    summary = OrderSummary(
        ...
        estimated_delivery_days=recommended_rate.max_delivery_days
    )
    ...
    return OrderReviewResponse(
        ...
        items=items_details,
        shipping_rate={
            "id": str(recommended_rate.id),
            "name": recommended_rate.name,
            "description": recommended_rate.description,
            "cost": float(recommended_rate.base_rate),
            "min_days": recommended_rate.min_delivery_days,
            "max_days": recommended_rate.max_delivery_days
        },
...
async def create_order(
    ...
    if order_data.shipping_address_id:
        ...
        user_query = select(User).where(User.email == order_data.customer_email)
    ...
    shipping_cost = shipping_rates[0].base_rate

    # Calculate discount
    discount_amount = Decimal("0.00")
    if order_data.promo_code and order_data.promo_code.upper() == "SAVE10":
        discount_amount = subtotal * Decimal("0.10")
